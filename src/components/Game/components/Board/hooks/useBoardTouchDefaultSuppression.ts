 ...
 * release → press-and-hold sequence opens the selection magnifier, which then
 * zooms the cells while the finger stays down. Neither
 * `-webkit-user-select: none`, `-webkit-touch-callout: none`,
 * `touch-action: none`, nor a viewport with `user-scalable=no` (ignored by iOS
 * Safari since iOS 10) suppresses it. The recognizer only stands down when the
 * page calls `preventDefault()` on a non-passive native `touchstart`.
 *
